...
public class Helpers {
    ...
    public static DataToSign getDataToSign(Container container, PKCS11SignatureToken token, SignatureProfile profile) {
        return SignatureBuilder.aSignature(container).
                withSigningCertificate(token.getCertificate()).withSignatureProfile(profile).buildDataToSign();
    }

    public static DataToSign getDataToSign(Container container, PKCS12SignatureToken token, SignatureProfile profile) {
        return SignatureBuilder.aSignature(container).
                withSigningCertificate(token.getCertificate()).withSignatureProfile(profile).buildDataToSign();
    }
